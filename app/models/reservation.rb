class Reservation < ApplicationRecord
  ...
  def check_minimum_cycle
    ...
    if selected_hosting
      ...
      if num_nights < min_cycle_amount * cycle_durations[cycle]
        errors.add(:num_nights,
                   "- the minimum number of nights to rent this property is #{min_cycle_amount * cycle_durations[cycle]}")
      end
    else
  ...
  def check_guest_capacity
    property = Property.find(property_id)
    return unless guests > property.guest_capacity

    errors.add(:guest_capacity,
               '- the number of guests exceeded the maximum allowed for this property.')
  end
